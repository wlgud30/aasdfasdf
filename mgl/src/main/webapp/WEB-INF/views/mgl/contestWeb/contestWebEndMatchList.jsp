...
<div class="tab_btn_b_area col_2data ver_lay_m">
	<div>
		<ul class="tab_btn_b wauto">
			...
			<li onclick="location.href='/ContestWeb/ContestWebEndMatchList.techni'" class="active_tab"><span>종료게임(점수수정)</span>
			</li>
	...
	<table class="text_td cet_lay th_mix">
		...
		<tbody>
			...
			<c:forEach var="list" items="${matchList}">
				<tr
					class="${list.ct_gidx},${list.team_a_idx},${list.team_b_idx},
								${list.ct_at_id1},${list.ct_at_id2},${list.ct_bt_id1},${list.ct_bt_id2},
								${list.team_a_score },${list.team_b_score }">
					...
					<td class="a1_nm popwin_btn0" id="${list.a1_age}-${list.a1_gd }"><p class="name3">${list.a1_nm}</p></td>
					<td class="a2_nm popwin_btn0" id="${list.a2_age}-${list.a2_gd }"><p class="name3">${list.a2_nm}</p></td>
					<td class="popwin_btn0 team_a_score">${list.team_a_score }</td>
					<td class="popwin_btn0 team_b_score">${list.team_b_score }</td>
					<td class="team_b_nm popwin_btn0" ><p class="club_name"><p class="club_name">${list.team_b_nm }</p></td>
					<td class="b1_nm popwin_btn0" id="${list.b1_age}-${list.b1_gd }"><p class="name3">${list.b1_nm }</p></td>
					<td class="b2_nm popwin_btn0" id="${list.b2_age}-${list.b2_gd }"><p class="name3">${list.b2_nm }</p></td>
					<td class="popwin_btn0">${list.ct_status }</td>
					<td onclick="location.href='#;'" title="출력"><i
						class="icon-printer title_ms main_color"></i></td>
				</tr>
				<input type="hidden"
					value="${list.ct_at_id1},${list.ct_at_id2},${list.ct_bt_id1},${list.ct_bt_id2},${list.ct_gidx},${list.team_a_idx},${list.team_b_idx},${list.ct_kind}" />
			</c:forEach>
		</tbody>
	</table>
</div>
				...
				<tr>
					<td class="club_name32"><span class="club_name" id="team_a_nm">리더아머마르스</span>
						...
						<div class="score_input_area">
							<input type="text"id="aScore" class="box_input cet_lay" placeholder="00"
								maxlength="2">
							<p class="tip">점수를 입력하세요</p>
						</div></td>
					<td class="score2_td"><span class="sign-round color3">진행</span>
						<ul class="td_st td_fix outcome">
							<li onclick="location.href='#;'" class="main_color">승</li>
							<li onclick="location.href='#;'" class="sub_color">패</li>
						</ul></td>
					<td class="club_name32"><span class="club_name" id="team_b_nm">TeamB.S.</span>
						...
						<div class="score_input_area">
							<input type="text" id="bScore" class="box_input cet_lay" placeholder="00"
								maxlength="2">
							<p class="tip">점수를 입력하세요</p>
						</div></td>
				</tr>
...
<input type="hidden" value="${list.ct_kind}" name="ct_kind" id="ct_kind2">
<script type="text/javascript">
	$(".popwin_btn0").click(function() {
		...
		$("#aScore").attr("placeholder",a_score); 
		$("#bScore").attr("placeholder",b_score);
		return false;
		...
		if (aScore == "" || bScore == "" || aScore == null || bScore == null) {
			...
			$.ajax({
				...
				data : JSON.stringify({
					...
					"aTeam" : $("#aTeam").val(),
					"bTeam" : $("#bTeam").val(),
					"ct_k_nm" : $("#ct_k_nm").text(),
					"ct_kind" : $("#ct_kind2").val()
				}),
				url : "/Contest/ContestPointUpdate.techni",
				dataType : "json",
				...
				success : function(data) {
					if (data.cnt > 0) {
						...
						alert("모든 예선경기가 끝나 점수 수정이 불가합니다.")
					}
				},
