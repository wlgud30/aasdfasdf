...
<div class="tab_btn_b_area">
	<ul class="tab_btn_b">
		<li onClick="location.href='/Cfight/CfightMyRank.techni'"
			class="active_tab"><span>나의 순위</span></li>
		<li onClick="location.href='/Cfight/CfightTypeRank.techni'"><span>종목별
				순위</span></li>
...
<c:choose>
				...
				<table class="text_td">
					<c:forEach items="${list }" var="list">
						<c:if test="${list.cf_t_nm eq list2.cf_t_nm }">
							<c:if
								test="${list.cf_team_a_id eq u_id || list.cf_team_b_id eq u_id }">
								<tr class="top_bg">
									<td class="no_td" id="${list.cf_t_idx}_${u_id}"></td>
									<td class="club_td"><span class="club_name">${list.c_nm }</span></td>
									<td class="n3_rank"><span class="name3">${list.a1_id }</span>
										<span class="age_rank">(${list.a1_age }-${list.a1_gd })</span></td>
									<td class="n3_rank"><span class="name3">${list.b1_id }</span>
										<span class="age_rank">(${list.b1_age }-${list.b1_gd })</span></td>
									<td class="point_td"><c:choose>
					...
					<c:forEach items="${list }" var="list">
						<c:if test="${list.cf_t_nm eq list2.cf_t_nm }">
							<tr>
								<td class="no_td" id="aa">${i}<input type="hidden"
									value="${i}" name="${list.cf_t_idx}_${list.cf_team_a_id}">
									<input type="hidden" value="${i}"
									name="${list.cf_t_idx}_${list.cf_team_b_id}">
								</td>
								<td class="club_td"><span class="club_name">${list.c_nm }</span></td>
								<td class="n3_rank"><span class="name3">${list.a1_id }</span>
									<span class="age_rank">(${list.a1_age }-${list.a1_gd })</span></td>
	...
	<c:otherwise>
		<div class="content white_bg">
			<h3 class="guide_big">
				<span class="icon-outline_4"></span> 참여한 게임이없습니다.
			</h3>
		</div>
	</c:otherwise>
</c:choose>
...
<div id="ft_area">
	...
	<ul id="ft_nv2" class="ft_menu">
		...
		<li onClick="location.href='/Cfight/CfightMyRank.techni'"
			class="active_tab"><span>순위</span></li>
