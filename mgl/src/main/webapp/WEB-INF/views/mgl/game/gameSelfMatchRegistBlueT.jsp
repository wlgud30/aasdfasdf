...
function selectChange(){
	var cs_k_idx = $("#wh").val();
	location.href="/Game/selfMatchBlueTeamInsertForm.techni?cs_k_idx="+cs_k_idx;
}
	...
	.then((value) => {
		if(value){
			$.ajax({
				async : true,
				type:"post",
				data : JSON.stringify({"cs_k_idx" : cs_k_idx , "cs_idx" : "${cs_idx}","u_id_a" : $("#id_1").val(),"u_id_b" : $("#id_2").val(), "bw" : "B"}),
...
<div class="tab_btn_b_area">
	<ul class="tab_btn_b">
		<li onClick="location.href='/Game/selfMatchTypeInsertForm.techni'"><span>조별(종목)</span>
		</li>
		<li onClick="location.href='/Game/selfMatchWBInsertForm.techni'"><span>선수구분</span>
		</li>
		<li class="active_tab" onClick="location.href='/Game/selfMatchBlueTeamInsertForm.techni'"><span>출전신청</span>
		</li>
	</ul>
</div>
	...
	<div>
		<ul class="tabs_cont">
			<li class="active_tab"><span
				onClick="location.href='/Game/selfMatchBlueTeamInsertForm.techni'">청팀</span></li>
			<li><span onClick="location.href='/Game/selfMatchWhiteTeamInsertForm.techni'">백팀</span></li>
		</ul>
	</div>
</div>
...
<div class="tit_select cet_lay">
	<ul class="li_col-nol">
		...
		<li>
			<h4 class="title_s">${list.size() }팀</h4>
		</li>
	</ul>
</div>
			...
			<tr>
				<td class="name_ph">
					<div class="photo_name">
						...
						<div class="text_area">
							<span class="name3">${list.cs_a_nm }</span> <span
								class="age_rank">(${list.cs_a_age }-${list.cs_a_grade })</span>
						</div>
					</div> <!--photo_name end-->
				</td>
				<td class="name_ph">
					<div class="photo_name">
						<div class="thum_area">
							<div class="thum"
								style="background-image: url(${list.cs_b_photo });"></div>
						</div>
						<div class="text_area">
							<span class="name3">${list.cs_b_nm }</span> <span
								class="age_rank">(${list.cs_b_age }-${list.cs_b_grade })</span>
						</div>
					</div> <!--photo_name end-->
				</td>
				<td class="btn_icons_td">
					<div class="icon_btn">
						<span onclick="teamDelete('${list.cs_t_idx}')" class="icon-remove"
							title="remove"></span>
					</div>
				</td>
			</tr>
...
<div class="content white_bg">
	<ul class="li_name line_tno">
		<c:forEach items="${list2}" var="list">
			<li
				onclick="te('${list.u_id }','${list.u_nm}','${list.u_photo}','${list.u_age}','${list.u_club_gd}','${list.u_sex }')"
				id="${list.u_id }">
				<div class="photo_name">
					<div class="thum_area">
					...
					<div class="text_area">
						<span class="name3">${list.u_nm}</span> <span class="age_rank">(${list.u_age}-${list.u_club_gd})</span>
					</div>
				</div> <!--photo_name end-->
			</li>
		</c:forEach>
...
<script>
	...
	$("#matchBecoming").click(function(){
		$.ajax({
			async : true,
			type:"post",
			data : JSON.stringify({"kind" : "청백전"}),
			url : "/Game/selfMatchGameInsert.techni",
			datatype : "json",
			contentType : "application/json; charset=UTF-8",
			success : function(data){
				if(data.cnt>0){
					location.href="/Game/selfMatchDetail.techni";
