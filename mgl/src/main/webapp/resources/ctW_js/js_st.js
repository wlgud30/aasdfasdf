$(window).on('load', function() {
  ...
  //메인메뉴
  var path_hr = window.location.pathname.split("/").pop(),
  sub_nav = $('.nav_sub > li p').filter('[onclick*="' + path_hr + '"]'),
  main_nav = $('#navi > li').children('p').filter('[onclick*="' + path_hr + '"]');
  sub_nav.addClass('active_navs');	
  sub_nav.closest('ul').siblings('p').addClass('active_nav');
  main_nav.addClass('active_nav');
...
jQuery(document).ready(function($) {
  ...
  if ($(head_el).length > 0) {
    ...
    $(head_el).each(function() {
      head_sum_h += $(this).outerHeight(true);
    });
  ...
  //container 높이 css
  var tie_head = $('header').next('.title_head'),
  ...
  $(accordion).on('click', '.accd_head', function() {
    ...
    if (element.hasClass('open')) {
      ...
      element.children('.accd_a').slideUp(200);
    } else {
      element.addClass('open');
      element.children('.accd_a').slideDown(200);
      element.siblings('li').children('.accd_a').slideUp(200);
  ...
  $(acd1).on('click', '.acd-head', function() {
    ...
    if (acd_p.hasClass('open')) {
      ...
      acd_p.find('.acd-cont').slideUp(200);
      acd_p.find('#map').stop(true, false).animate({
        opacity: 0
      }, 20);
    } else {
      acd_p.addClass('open');
      acd_p.find('.acd-cont').slideDown(200);
      acd_p.find('#map').stop(true, false).animate({
        opacity: 1
      }, 20);
    }
  ...
  $('input[name^="option_add"]').change(function() {
    ...
    if ($('#check_detail').is(':checked')) {
      $(check_uls).slideDown(200);
    } else {
      $(check_uls).slideUp(200);
    }
  });
  ...
  $(pop_btn).on('click', function(e) {
    ...
    if (!$($popup_nb).parent().is($popup_wp)) {
      ...
      $($popup_nb).show().stop(true, false).animate({
        opacity: 1
      }, 200);
    }
    $(window).on('load resize', function() {
    ...
    $(popup_wrap).click(function() {
      if ($(this).children().parent().is(popup_wrap)) {
        $(this).children().unwrap().hide().stop(true, false).animate({
          opacity: 0
        }, 200);
      }
      $($menu).each(function() {
    ...
    $(popup_wrap).find(".la-close, .close_popup").click(function() {
      if ($($popup_nb).parent().is(popup_wrap)) {
        $($popup_nb).unwrap().hide().stop(true, false).animate({
          opacity: 0
        }, 200);
      }
      $($menu).each(function() {
    ...
    }); //$(popup_wrap).find(".la-close, -> end
  }); //$(pop_btn).on('click' -> end
  ...
  if ($one_over.length > 0) {
    ...
    function destroyDots() {
      $one_over.trigger('destroy');
    }
    createDots();

    $('.one_over_view').on('click', function() {
      var one_over = $(this).next($one_over),
        one_over_p = $(this).closest('.one_over_area');

      one_over_p.toggleClass('one_opened');
      if (one_over_p.hasClass('one_opened')) {
        destroyDots();
      } else {
        createDots();
      }
