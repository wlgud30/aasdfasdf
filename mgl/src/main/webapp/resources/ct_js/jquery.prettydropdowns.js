  ...
  $.fn.prettyDropdown = function(oOptions) {
    ...
    var $current,
      ...
      init = function(elSel) {
        ...
        $select.css({'visibility':'hidden','display':'none'}).outerHeight(oOptions.height);
        nTimestamp = +new Date();
        ...
        var $items = $('optgroup, option', $select),
          ...
          sHtml = '<ul' + (elSel.disabled ? '' : ' tabindex="0"') + ' role="listbox"'
            ...
            // + ' style="max-height:' + (oOptions.height-2) + 'px;">';
        ...
        if ($dropdown.width()<=0) {
          ...
          nOuterWidth = $clone.children('ul').outerWidth()-8;
          $clone.remove();
        }
        // Set dropdown width and event handler
        // NOTE: Setting width using width(), then css() because width() only can return a float,
        // which can result in a missing right border when there is a scrollbar.
        $items.width(nWidth).css('width', $items.outerWidth()-8).click(function() {
          var $li = $(this),
            ...
            if (nDropdownBottom>nWinHeight) {
              ...
              if (nOffsetTop-nScrollTop>nWinHeight-(nOffsetTop-nScrollTop+oOptions.height)) {
                ...
                if (nOffsetTop-nScrollTop+oOptions.height<nDropdownHeight) {
                  $dropdown.outerHeight(nOffsetTop-nScrollTop+oOptions.height-78);
                  // Ensure the selected item is in view
                  $dropdown.scrollTop(nDropdownHeight);
                }
              } else {
                $dropdown.height($dropdown.height()-(nDropdownBottom-nWinHeight) - 48);
              }
            }
